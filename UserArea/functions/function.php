...
<div  class="">
<div class="card mb-3 " style="max-width: 550px; min-height:500px; border-radius:10px;">
  <div class="row g-0 ">
    <div class=" col-md-4">
      <img style="min-height:530px; max-width:100%; border-radius:10px;" src="img/'.$img.'"  alt="...">
    </div>
    <div class="col-md-8">
      <div class=" mt-0 m-1  card-body">
        ...
        <p class="m-3 card-text fs-5">'.$food_type.'</p>
         ...
         <div class="price">
          <a href="cart.php?add_to_cart='.$food_id.'"><button type="button" class="m-0 w-100 mt-4 btn btn-outline-warning"><i class="fa-solid fa-cart-shopping"></i></button></a>
          <a href="food_details.php?details_id='.$food_id.'"><button type="button" class="m-0 w-100 mt-4 btn btn-outline-warning"><i class="fa-solid fa-eye"></i></button></a>
        </div>
      </div>
    </div>
  </div>
...
function all_food(){
  ...
  <div  >
    <div class="test-card card mb-3 " style="max-width: 565px; min-height:500px; border-radius:10px;">
      <div class="row g-0 ">
        <div class=" col-md-4">
          <img style="min-height:530px; max-width:100%; border-radius:10px;" src="img/'.$img.'"  alt="...">
        </div>
...
<div  class="">
  <div class="card mb-5"  style="max-width: 550px; min-height:500px; border-radius:10px;">
    <div class="row g-0 ">
      <div class=" col-md-4">
        <img style="min-height:530px; max-width:100%; border-radius:10px;" src="img/'.$img.'"  alt="...">
      </div>
      <div class="col-md-8">
        <div class=" mt-0 m-0  card-body">
          <h5 class="m-2 card-title fw-bold fs-3">'.$food_name.'</h5>
          <p class="m-3 card-text fs-5">'.$food_country.'</p>
          <p class="m-3 card-text fs-5">'.$food_type.'</p>
           <p class=" m-3 card-text">'.$food_decription.'</p>
...
function show_order(){
  ...
  if($number_order>0){
    while($row_order=mysqli_fetch_assoc($result_order)){
      ...
      $select_food="SELECT * FROM food_order WHERE(Food_id=$Food_id)";
      $result_food=mysqli_query($con,$select_food);
      ...
      if($number_food>0){
        while($row_food=mysqli_fetch_assoc($result_food)){
          ...
          echo'
          <div class="col-xxl-4">
    <div class=" card mb-4" style="width:100%; height:480px; ">
  ...
  if(isset($_POST['Send_feed'])){
    ...
    if(isset($_SESSION['id'])){
      ...
      if($result){
        ...
        $insert_user="INSERT INTO feedback (User_id,User_name,User_email,Feedback) VALUES ($user_id,'$User_name','$User_email','$massage')";
      $result_insert_user=mysqli_query($con,$insert_user);
      if($result_insert_user){
        echo '<script>alert("Thanks for Maasage")</script>';
        echo '<script>window.open("home.php","_self")</script>';
      }
      }
      }
      ...
      $insert_feed="INSERT INTO feedback (User_id,User_name,User_email,Feedback) VALUES ('','$user_name','$email','$massage')";
      $result_feed=mysqli_query($con,$insert_feed);
      if($result_feed){
        echo '<script>alert("Thanks for Maasage")</script>';
        echo '<script>window.open("home.php","_self")</script>';
      }
...
<?php
function show_number_feedback(){
global $con;
$select_feedback="SELECT * FROM feedback";
$result_feedback=mysqli_query($con,$select_feedback);
$number_feedback=mysqli_num_rows($result_feedback);
return $number_feedback;
}
?>
...
<?php
function show_number_dishes(){
 global $con;
 $user_id=$_SESSION['id'];
 $totla_dish=0;
 $select_dish="SELECT * FROM ordres_user WHERE (User_id=$user_id)";
 $result_select_dish=mysqli_query($con,$select_dish);
 if($result_select_dish){
  while($row_fetch=mysqli_fetch_assoc($result_select_dish)){
    $quntity_dish=array($row_fetch['quantity']);
    $value=array_sum($quntity_dish);
    $totla_dish+=$value;
...
<?php
function user_number_feedback(){
  global $con;
 $user_id=$_SESSION['id'];
 $select_feedback="SELECT * FROM feedback WHERE (User_id=$user_id)";
 $result_feedback=mysqli_query($con,$select_feedback);
 $number_feedback=mysqli_num_rows($result_feedback);
 return $number_feedback;
}
